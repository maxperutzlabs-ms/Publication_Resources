---
title: "Filter, Impurity-correct, Normalize MaxQuant proteinGroups.txt"
...
This script processes MaxQuant proteingroups.txt output coming from isobaric labeling experiments. It performs filtering out reverse hits, contaminants and peptides only identified by site, as well as filtering based on valid values and intensity. Further, isobaric impurity correction is performed; and finally, the intensity columns are normalized between samples. 
...
## Specify file path to impurity matrix
impurity_path = "impurity_matrix_tmtpro.csv" 

## Specify groups vector
groups = c("126C", "blank", "blank", "Group0", "Group6", "Group0", "Group6", "Group0", "Group6", "blank", "Group9", "Group12", "Group9","Group12", "Group9", "Group12")
## Specify sample name vector
samplenames = c("126C", "blank", "blank", "Group0.1", "Group6.1", "Group0.2", "Group6.2", "Group0.3", "Group6.3", "blank", "Group9.1", "Group12.1", "Group9.2","Group12.2", "Group9.3", "Group12.3")

## Optional: Specify the group names that represent empty channels (e.g. in case when employing 16plex kit but only 12 channels are used for labeling in the experiment). Their reporter ion columns will be filtered out. This parameter requires specification of the empty groups as a character vector. If no group is empty, specify as NULL.
groups_empty = c("126C", "blank")
...
## specify intensity column pattern as regular expression pattern. 
intensityCol_pattern = "Reporter[.]intensity[.][0-9]+[.]MS3_prot$"

## Specify normalization method (either "loess" or "median")
norm_method <- "loess"

## Optional: Specify which groups should be included in a 1-way ANOVA. if NULL, all groups will be used
groups_anova <- NULL

## Specify name of the output file string (without .txt extension). If NULL, will be called according to input file string
output_name <- "proteinGroups_MS3"


## note: Parameters that are described as "Optional" can be ignored if certain steps are to be skipped. 
## note: If specified correctly, no other user input should be required for the remainder of the script
## note: If the script produces errors, please check the whether all input parameters were specified correctly.
...
## make sure that batch is a character
if(!is.null(batch)){
...
```{r Read in data, filter out duplicate intensity columns, echo=FALSE, warning=FALSE}

## Read in file
df <-  read.delim(filename_path, sep="\t", header=TRUE)
writeLines("Dimensions of df: ")
dim(df)
...
## Extract intensity columns by smatching to specific pattern: "Reporter[.]intensity[.][0-9]*$". These reporter ions columns will be used in this script!
...
## Replace old reporter names with uniform name
...
```{r Filter out contaminants and reverse and for at least 2 razor+unique peptides , echo=FALSE, warning=FALSE}
...
df_corr <- impurity_correction(df = df, 
                                   ...
                                   impuritymatrix = as.matrix(read.csv(file=impurity_path, sep=",", header=TRUE, row.names = 1)))
...
```{r Filter proteins based on intensity, echo=FALSE, warning=FALSE}
...
writeLines(paste0("Dimensions before filtering: ", dim(df_corr_filtered_norm)))
...
## Check where the cutoff would lie
...
writeLines(paste0("\nDimensions after filtering: ", dim(df_corr_filtered_norm)))

## Investigate NAs in remaining and filtered data
writeLines("\nHow many NAs in remaining data:")
table(is.na(df_above_int_filter))
writeLines("\nHow many NAs in filtered data:")
table(is.na(df_below_int_filter))
...
heatmap_plot(m=t(scale(t(log2(m_reporter)), scale=FALSE)),
         groups = groups,
         samplenames = samplenames,
         legend_colors = colors_groups, type="centered")
...
```{r Optional: Differential Expression testing 1, echo=FALSE, warning=FALSE}
...
if (is.null(output_name)){
  ...
  outputname_path <- paste0("./filtered_normalized_",substring(filename_path, first = ind_substring + 1, last=nchar(filename_path)))
} else {
  outputname_path <- paste0("./",output_name,".txt")
}
